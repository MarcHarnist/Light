...
// Supprime l'objet Message s'il existe encore
if (isset($message))
    unset($message);

// Cherche si une variable post existe
if (isset($_POST) && ! empty($_POST) && isset($_POST['operation'])) {
    // Crée un message vide dans une variable de type string (chaîne de caractères)
    ...
    if ($_POST['operation'] == "creation") {
        ...
        if (isset($_POST['nom_de_la_table']) && null !== $_POST['nom_de_la_table'] && ! empty($_POST['nom_de_la_table']) && $_POST['nom_de_la_table'] !== '') {
            ...
            elseif (Methods::cleanSqlTableName($nom_de_la_table) === false) {
                // Crée un message d'erreur
                $message .= " <span style=\"color: red\">Le nom doit comporter uniquement des lettres, des chiffres et des underscores (_).</span>";
            ...
            elseif ($database->table_exists($nom_de_la_table) === true) {
                ...
                $post = array(
                    ...
                    "author" => '',
                    "title" => 'Succès',
                    "text" => 'Voici votre première page. Vous pouvez la modifier !',
                    "category" => 'news',
                    "category_new" => '',
                    "operation" => "creation"
                );
