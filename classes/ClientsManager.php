...
class ClientsManager extends Database{
  ...
  public function __construct()
  {
    ...
    $q = $this->db->prepare('INSERT INTO ' . self::TABLE_CLIENT . '(civilite, name, firstname, email, password, phone, level) VALUES(:civilite, :name, :firstname, :email, :password, :phone, :level)');
  ...
  {
    return $this->db->query('SELECT COUNT(*) FROM ' . self::TABLE_CLIENT . '')->fetchColumn();
  }
  ...
  public function exists($info)
  {
    ...
    {
      return (bool) $this->db->query('SELECT COUNT(*) FROM ' . self::TABLE_CLIENT . ' WHERE id = '.$info)->fetchColumn();
    }
    // Sinon, c'est qu'on veut vérifier que le name existe ou pas.
    $q = $this->db->prepare('SELECT COUNT(*) FROM ' . self::TABLE_CLIENT . ' WHERE name = :name');
    $q->execute([':name' => $info]);
  ...
  public function get($info)
  {
    ...
    {
      $q = $this->db->query('SELECT * FROM ' . self::TABLE_CLIENT . ' WHERE id = '.$info);
    ...
    elseif(is_string($info)) // is_string: Get client attributs by "name" (string)
    {
		...
		$requete = 'SELECT * FROM ' . self::TABLE_CLIENT . ' WHERE name = "' . $info . '"';
  ...
  {
    $clients = [];
    $q = $this->db->prepare('SELECT * FROM ' . self::TABLE_CLIENT . ' WHERE name <> :name ORDER BY name');
    $q->execute([':name' => $name]);
