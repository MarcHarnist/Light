...
*   Class Mydatabase
*
*   File     : classes/Mydatabase.php
*   Author   : Marc L. Harnist
*   Creation : 2020-07-03
*   Updated  : 
*
*   Extends Methods extends Website
*
*   Constants and variables in the class Website ;
*   No fixed values here but only methodes ;
*
*       METHODES
*   __construct()...................51
*
...
*
*/
class Mydatabase extends Methods // extends Methods extends Website
{
    protected $db;
...
/** Nom : create_in_table_budget_rules()
    ...
    *     Updated : 2020-07-03
    */
      function read_table_databall(string $table, $value, $attribut)
	  {
		$messageHeader = " Message du fichier class/Mydatabase : ";
		if(empty($table)) return $messageHeader . "Le nom de la table est vide. (Ligne ". __LINE__ .")";
		if(empty($value)) return $messageHeader . "Valeur (\$value) est vide. (Ligne ". __LINE__ .")";
		if(empty($attribut)) return $messageHeader . "Attributs (\$attribut) vides. (Ligne ". __LINE__ .")";
		  
        $mysql_request = 'SELECT * FROM ' . $table . ' where ' . ' value = ' . $value .' and attribut = ' . $attribut;
		
		//Afficher la requête dans un var_dump()
		// var_dump($mysql_request);
		
		//Try to execute the request and catch an error message in case
		try
		{
			$stmt = $this->db->query($mysql_request); //stmt = preparing statement, (état de préparation: convention de codeur)
