...
*/  public function clientsPermissions($int = 99, $client){
		if(!$_SESSION['client'])
			header('Location: ' . $this->page_url . 'accueil');//Redirection au tableau-de-bord des clients
		elseif($client !== Null && $client->level() > $int)//Si le visiteur n'a pas le niveau (droits)
			header('Location: ' . $this->page_url . 'acces-limite');//Redirection à l'accueil du site
    }
